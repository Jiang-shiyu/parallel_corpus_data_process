...
def count_confreq(en_words_freq, jp_words_freq,lemmatized_en_sentences, lemmatized_jp_sentences, outputfilename):
	pairs=make_pairs(en_words_freq, jp_words_freq)
	combine_sentence_lists=combine_en_jp(lemmatized_en_sentences, lemmatized_jp_sentences)
	start = time.time()
	confreq={}
	for en,jp in pairs:
		confreq[(jp,en)]=0
	...
	print(str(end-start))
	pd.Series(confreq_dic).to_csv(outputfilename)
	return confreq
